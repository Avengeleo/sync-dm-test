...
def _pull_until(offline_http, msg_id, timeout=12):
...
def test_single_normal_offline_pull(logged_in_client, offline_http):
    ...
    assert hit["from_id"] == a.user_id, "离线行 sFromId 应为发送者"
...
def test_single_reaction_offline_pull(logged_in_client, offline_http):
    ...
    assert hit["parent_msg_id"] == parent, "回应离线行 parentMsgId 应为被回应消息 id"
